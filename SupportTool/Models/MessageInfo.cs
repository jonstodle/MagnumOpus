...
namespace SupportTool.Models
{
	...
	{
		public static IDisposable RegisterPromptHandler(this Interaction<MessageInfo, int> source, Grid parent) => source.RegisterHandler(async interaction => { interaction.SetOutput(await new DialogControl(parent, interaction.Input.Caption.HasValue() ? interaction.Input.Caption : null, interaction.Input.Message, interaction.Input.Buttons).Result.Take(1)); });
